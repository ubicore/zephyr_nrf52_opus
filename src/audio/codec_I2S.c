...
#define SGTL5000_PIN_3V        (32+6)


#undef NRFX_I2S_CONFIG_IRQ_PRIORITY
#define NRFX_I2S_CONFIG_IRQ_PRIORITY 5


static struct k_msgq *p_msgq_I2S_Rx;
static struct k_msgq *p_msgq_I2S_Tx;

#define AUDIO_FRAME_WORDS                   CONFIG_AUDIO_FRAME_SIZE_SAMPLES // Sample Left and right in one word
...
static bool i2s_sgtl5000_driver_evt_handler(drv_sgtl5000_evt_t * p_evt)
{
	PCM_Rx_t PCM_Rx;
	PCM_Tx_t PCM_Tx;
	PCM_Rx_t * p_PCM_Rx = &PCM_Rx;
	PCM_Tx_t * p_PCM_Tx = &PCM_Tx;
...
#if (CONFIG_CHANNELS_RX == 1)

                    	//Stereo to Mono : copy left sample only
                    	for(int i=0; i< received;i++){
                    		//Copy sample
                    		p_PCM_Rx->buff[i] = p_buffer[i*2];
                    	}
#elif (CONFIG_CHANNELS_RX == 2)
                		opus_mempcpy(PCM_Rx->buff, p_buffer, received*sizeof(uint32_t));
#else
#error "Not supported"
#endif

                		/* capture initial time stamp */
                		p_PCM_Rx->timestamp_I2S_Out = k_uptime_get_32();

                    	if(k_msgq_put(p_msgq_I2S_Rx, p_PCM_Rx, K_NO_WAIT) != 0){
                    		audio_stats_counter.I2S_Rx_discared++;
                    	}
                    }
       			}else {
       				/*Discard recorded PCM buffer*/
       			}
            }
            break;
...
//        		if(audio_get_role() == player){
                	if(k_msgq_get(p_msgq_I2S_Tx, p_PCM_Tx, K_NO_WAIT) == 0){
#if (CONFIG_CHANNELS_TX == 1)
                		//Mono to Stereo
                		for(int i=0; i< (i2s_buffer_size_words);i++){
                			//Copy sample
                			p_buffer[i*2] = p_PCM_Tx->buff[i];
                			p_buffer[i*2+1] = p_PCM_Tx->buff[i];
                		}
#elif (CONFIG_CHANNELS_TX == 2)
                		opus_mempcpy(p_buffer, PCM_Data_TX, i2s_buffer_size_words*sizeof(uint32_t));
#endif
                		/* capture time stamp */
                		p_PCM_Tx->timestamp_play.I2S_In = k_uptime_get_32();
                		/*update latency*/
                		latency_player_update(&p_PCM_Tx->timestamp_play);
                	}else{
                		audio_stats_counter.I2S_Tx_missing++;
                	}
//        		}else{
//                    /* Clear pcm buffer */
...
void audio_codec_I2S_connect(struct k_msgq *p_msgq_Rx, struct k_msgq *p_msgq_Tx)
{
	p_msgq_I2S_Rx = p_msgq_Rx;
	p_msgq_I2S_Tx = p_msgq_Tx;
}

void audio_codec_I2S_init()
{
    uint32_t                err_code;
    ...
    nrf_gpio_pin_clear(SGTL5000_PIN_3V);
    k_sleep(K_MSEC(1));
    nrf_gpio_pin_set(SGTL5000_PIN_3V);
    ...
    LOG_INF("I2S initialization done.");
}
